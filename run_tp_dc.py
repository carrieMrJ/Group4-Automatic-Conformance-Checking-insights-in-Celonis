from src.data_integration.get_data import trace_cluster, split_df, \
    calculate_temporal_profile_task_duration, calculate_temporal_profile_temporal_distance, encode_activities
...
from src.anomaly_detection.get_datas import get_data_for_anomaly_detection_receipt, \
    get_data_for_anomaly_detection_review
...
def temporal_profile_analysis(data_model, table_name, case_column, activity_column, time_column,
    ...
    temporal_profile_task_dur_all, all_task_dur = calculate_temporal_profile_task_duration(data_model, table_name,
                                                                                           ...
                                                                                           mainstream_case_id=mainstream_id)
...
def anomaly_detection(data_model, table_name, case_column_name, act_column_name, res_column_name, time_column_name):
    if table_name == 'reviewing':
        df = get_data_for_anomaly_detection_review(data_model, table_name, case_column_name, act_column_name,
                                                   res_column_name, time_column_name)
    if table_name == 'receipt':
        df = get_data_for_anomaly_detection_receipt(data_model, table_name, case_column_name, act_column_name,
                                                    res_column_name, time_column_name)
